...
function Navigation({ name, handleRegisterOpen, onClose, logout, isLoggedIn, isOpen }) {
  ...
  return (
    <section className={`popup-box popup-box_type_nav ${isOpen ? 'popup-box_opened' : ''}`}>
      <div className="popup-box__container popup-box__container_type_nav">
        ...
        <Header handleRegisterOpen={handleRegisterOpen} onClose={onClose} name={name} isLoggedIn={isLoggedIn} isHome={true} logout={logout} isNavBar={true} />
      </div>
    </section>
  )
